...
@router.get("/{book_slug}", response_model=list[ChapterPublic])
async def get_chapter(session: SessionDep, book_slug: str, index: int | None = None):
    ...
    if index is not None:
        ...
        if not chapter:
            raise http_exc_404_chapter_not_found_request(chapter_id=f"{index}")
        return [chapter]
    chapters = await crud_chapter.get_chapters_by_book_id(session, existing_book.id)
    return chapters

@router.post("/{book_slug}", response_model=ChapterPublic, status_code=status.HTTP_201_CREATED)
async def create_chapter(session: SessionDep, book_slug: str, chapter_data: ChapterRegister, current_admin: CurrentAdmin):
    existing_book = await crud_book.get_book_by_slug(session, book_slug)
    if not existing_book:
        raise http_exc_404_book_not_found_request(string=f"{book_slug}")
...
@router.get("/content/{book_slug}/{index}", response_model=ChapterContentPublic)
async def get_chapter_content(session: SessionDep, book_slug: str, index: int):
...
@router.post("/content/{book_slug}/{index}", response_model=ChapterContentPublic, status_code=status.HTTP_201_CREATED)
async def create_chapter_content(session: SessionDep, book_slug: str, index: int, chapter_content_data: ChapterContentRegister, current_admin: CurrentAdmin):
    book = await crud_book.get_book_by_slug(session, book_slug)
    if not book:
        raise http_exc_404_book_not_found_request(string=f"{book_slug}")
    chapter = await crud_chapter.get_chapter_by_book_id_and_chapter_index(session, book.id, index)
...
@router.patch("/content/{book_slug}/{index}", response_model=ChapterContentPublic)
async def update_chapter_content(session: SessionDep, book_slug: str, index: int, chapter_content_data: ChapterContentUpdate, current_admin: CurrentAdmin):
    book = await crud_book.get_book_by_slug(session, book_slug)
    if not book:
        raise http_exc_404_book_not_found_request(string=f"{book_slug}")
    chapter = await crud_chapter.get_chapter_by_book_id_and_chapter_index(session, book.id, index)
